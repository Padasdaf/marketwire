...
const router = useRouter()
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      const articles_response = await fetch(`https://financialmodelingprep.com/api/v3/fmp/articles?page=0&size=5&apikey=${process.env.NEXT_PUBLIC_FMP_API_KEY}`)
  ...
  return (
    <main className="flex-1">
      <div className="container px-10 py-5 ">
        <h1 className="text-4xl pb-8 text-[#438B22] text-pretty font-semibold drop-shadow-md ">Your Stocks</h1>
        <DataTable columns={columns} data={tableData} onRowSelect={handleRowSelect} />
      </div>
      <div className="container px-10 py-5 text-center gap-5">
        <h1 className="text-4xl pb-8 text-[#438B22] drop-shadow-md">Whats on the News?</h1>
